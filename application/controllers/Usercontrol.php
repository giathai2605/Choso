...
class Usercontrol extends MY_Controller {
		...
		if($totals){
			...
			if ($totals['integration']['all'] ==null) {
				...
				foreach ($totals['integration']['all'] as $website => $value) {
					...
					<li class="list-group-item">
					<a class="btn btn-lg btn-default btn-success" href="http://'. $website .'" target="_blank">'. __( 'admin.preview_store' ) .'</a>
					</li>
					</ul>
					</div>';
				}
	...
	public function store_markettools(){
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST')
		{
			$escapevendors = $this->db->query('SELECT user_id,vendor_shares_sales_status FROM vendor_setting WHERE vendor_shares_sales_status>0')->result_array();
			...
			$json['tools'] = $this->IntegrationModel->getProgramTools($filtertools);
 			...
 			$restricted_vendors = $this->get_restricted_vendors();
			$escapevendors = $this->db->query('SELECT user_id,vendor_status FROM vendor_setting WHERE vendor_status>0')->result_array();
...
public function exportproductXML(){
	...
	if($structure_only == 1) 
	{
		...
		$dom->save(APPPATH.'/core/excel/output/export_vendor_products_structure.xml');
		$json['download'] = base_url('application/core/excel/output/export_vendor_products_structure.xml');
	...
	else
	{
		...
		$dom->appendChild($root);
		$dom->save(APPPATH.'/core/excel/output/export_vendor_products.xml');
		$json['download'] = base_url('application/core/excel/output/export_vendor_products.xml');	
	}
...
public function downloadprodcutxmlstructurefile($filename = NULL) {
    ...
    $data = file_get_contents(base_url('application/core/excel/output/export_vendor_products_structure.xml'));
...
public function downloadprodcutxmlfile($filename = NULL) {
	...
	if(empty($userdetails)) redirect('usercontrol/dashboard');
    $this->load->helper('download');
    $data = file_get_contents(base_url('application/core/excel/output/export_vendor_products.xml'));
...
public function deleteALLSaleproducts(){
			...
			public function confirm_deposit(){
				...
				if($vendorDepositStatus['depositstatus']){
					...
					} else if((int)$data['amount'] > 0)
					{
						...
						if($vendor_deposit["vd_amount"]>=$requiredeposit)
						{
						...
						else
						{
							...
							$json['error'] = "<div class='alert alert-danger'>".$this->session->userdata('userCurrency')." ". $requiredeposit. " " .__('user.minimum_deposit_amount_required')."</div>";
						}
					...
					if($this->form_validation->run()){
						...
						if(!isset($errors) || empty($errors)) {
							...
							if($is_generate) {
								...
								$json['success'] = 'Ticket Generated Successfully';
							} else {
								$json['errors'] = 'Please try again';
							}
						} else {
							$json['errors']['attachment'] = $errors;
						}
	...
	public function exportReviewXML(){
		...
		if($structure_only == 1) 
		{
			...
			$root->appendChild($product_node);
			$dom->appendChild($root);
			$dom->save(APPPATH.'/core/excel/output/export_vendor_product_reviews_structure.xml');
			$json['download'] = base_url('application/core/excel/output/export_vendor_product_reviews_structure.xml');
		...
		else
		{
			...
			$dom->appendChild($root);
			$dom->save(APPPATH.'/core/excel/output/export_vendor_product_reviews.xml');
			$json['download'] = base_url('application/core/excel/output/export_vendor_product_reviews.xml');	
		}
	...
	public function downloadproductreviewxmlstructurefile($filename = NULL) {
	   $userdetails = $this->userdetails();
		if(empty($userdetails)){ redirect('/login'); }
	    $this->load->helper('download');
	    $data = file_get_contents(base_url('application/core/excel/output/export_vendor_product_reviews_structure.xml'));
	...
	public function downloadproductreviewxmlfile($filename = NULL) {
		...
		if(empty($userdetails)){ redirect('/login'); }
	    $this->load->helper('download');
	    $data = file_get_contents(base_url('application/core/excel/output/export_vendor_product_reviews.xml'));
