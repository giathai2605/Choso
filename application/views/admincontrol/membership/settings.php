<?php if(!empty($license_alret)) { ?>
    <h5 class="notification_on_pages">
		<div class="mybadge text-wrap rounded badge-danger mb-2">
			<?= $license_alret; ?>
		</div>
	</h5>
<?php } ?>
				...
				<div class="col-sm-12">
					...
					<div class="tab-content">
						<div class="tab-pane p-3 active" id="tab-setting" role="tabpanel">
							...
							?>
							<div class="form-group">
								<label class="control-label"><?= __('admin.default_plan_for_new_affiliates') ?></label>
								<select class="form-control" name="membership[default_affiliate_plan_id]">
							...
							<div class="form-group">
								<label class="control-label"><?= __('admin.default_plan_for_new_vendors') ?></label>
								<select class="form-control" name="membership[default_vendor_plan_id]">
									<option value=""><?= __('admin.none') ?></option>
									<?php foreach ($plans as $key => $plan) { 
										if($plan->user_type == 2) {
											?>
