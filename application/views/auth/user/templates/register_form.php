...
<ul id="registartion_type" class="nav nav-pills mt-4" role="tablist">
	...
	if((int)$vendor_storestatus==0 && (int)$vendor_marketstatus==0 ) {} else { ?>
 
	<?php if($registration_status == 1 || $registration_status == 2): ?>
		<li class="nav-item text-center border">
...
<form action="" method="POST" role="form" class="reg_form p-3" novalidate="" enctype='multipart/form-data'>
	...
	<?php
		...
		if(isset($read_only_user_membership_plan)){
			...
			$fields['user_membership_plan'] = 
						'<div class="form-group">
							<label>'. __('admin.user_membership_plan') .'</label>
		...
		if (isset($allow_vendor_option)) {
			...
			$fields['is_vendor'] = '<div class="form-group">
				<label>'. __('admin.vendorstatus') .'</label>
				<div>
					<input name="is_vendor" class="btn-switch update_all_settings" type="checkbox" '. ($is_vendor ==1 ? 'checked' : '') .' data-toggle="toggle" data-size="normal" data-on="'.__('admin.status_on').'" data-off="'.__('admin.status_off').'">
				</div>
			</div>';
		}
	...
	<?php if(!isset($edit_view)){ ?>
		...
		<?php if(isset($template_index)) { ?>
			<div class="check_box text-left">
				<p>
					...
					<span for="checkbox">
						<a href="#" data-toggle="modal" data-target="#termOfUse" style="margin-top:0px; color: black;">
		...
		<!-- <input class="<?= $allow_back_to_login ? "btn btn-primary" : ""; ?> btn-registration btn-submit btn continue_btn px-4 text-white round_btn d-block mx-auto front_button_color front_button_hover_color front_button_text_color" type="submit" value="<?= __('user.submit') ?>"> -->
