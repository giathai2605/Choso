<?php foreach ($transaction as $key => $value) { ?>
	...
	<tr class="main-tr <?= $recurring ? 'recurring recurringof-'.$recurring : '' ?>">
		...
		<td>
			...
			<div class="wallet-popover badge badge-secondary py-1 pl-2 font-14" toggle="popover"> 
		...
		<td class="text-center">
			...
			<?php if(false && $value['status'] == "1"){ ?>
				...
				<?php } else { ?>
					<button class="btn btn-primary " data-toggle="modal" href='#withdrawal-limit'><?= __('user.send_request') ?></button>
				<?php } ?>
			<?php } else {
		...
		<td colspan="4" class="text-right">
			<?php if(!$value['parent_id']){  ?>
	    		<?php if($value['wallet_recursion_id']){ ?>
	    			<span class="badge badge-default p-2"><?= __('user.runs') ?> <?= (int)$value['total_recurring'] ?> <?= __('user.cycle_and_next_is_at') ?> <?= $value['wallet_recursion_next_transaction'] ?></span>
	    		<?php } ?>
	    	<?php } ?>
